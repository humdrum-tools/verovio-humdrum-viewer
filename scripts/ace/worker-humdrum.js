    ...
    function validateHumdrum_Process(hum, onError, onWarning) {
      ...
      const REGEXP_EMPTY = /^\s*$/;
          ...
          function process_local(fields) {
            var position = 0;
            if (new_path) {
              if (!onError("Record containing add-spine indicator has not followed by exclusive interpretation for that spine", i, 0)) return;
              new_path = false;
            };
            if (!first_interpretation_yet) {
              if (!onError("Local comment precedes first exclusive interpretation record", i, 0)) return;
              current_no_of_spines = fields.length;
            };
            if (fields.length !== current_no_of_spines) {
              if (!onError("Number of sub-comments in local comment does not match the number of currently active spines", i, 0)) return;
            };
            ...
            for(var j = 0; j < interp.length; j++) {
              if (interp[j].indicator === "N") {
                if (!REGEXP_EXCLUSIVE.test(fields[j])) {
                  if (!onError("Record containing add-spine indicator has not been followed by exclusive interpretation for that spine", i, position)) return;
                };
                interp[i].indicator = "*";
              };
              position += fields[j].length;
          ...
          function check_first_interp(fields) {
            ...
            for (var j = 0 ; j < fields.length; j++) {
              if (fields[j] === "*") {
                if (!onError("First exclusive interpretation record contains a null interpretation", i, position)) return;
              };
              if (!REGEXP_EXCLUSIVE.test(fields[j])) {
                if (!onError("First exclusive interpretation record contains a non-exclusive interpretation", i, position)) return;
              };
              if (fields[j].charAt(0) === "*" && (SPINE_CHARS.indexOf(fields[j].charAt(1)) >= 0)) {
                if (!onError("First exclusive interpretation record contains a spine-path indicator", i, position)) return;
              };
              position += fields[j].length;
            };
          };
            ...
            for(j = 0; j < interp.length; j++) {
              ...
              switch(current.indicator) {
                ...
                case "x": //do_exchange
                  ...
                  if (error || exchange < 0) {
                    if (!onError("Improper number of exchange-path indicators", i, 0)) return;
                  } else {
                    temp = current.name;
                ...
                case "v": //do_join
                  ...
                  if (j === interp.length - 1) {
                    if (!onError("Single join-path indicator found at end of interpretation record", i, 0)) return;
                  } else if (interp[j + 1].indicator !== 'v'){
                    if (!onError("Join-path indicator is not adjacent to another join-path indicator", i, 0)) return;
                  } else {
                    j = j + 1;
                    while (j < interp.length && interp[j].indicator === "v") {
                      if (current.name !== interp[j].name) {
                        if (!onError("Exclusive interpretations do not match for designated join spines", i, 0)) return
                      };
                      interp.splice(j, 1);
                      current_no_of_spines--;
                    };
          ...
          function process_interpretation(fields) {
            ...
            for (var j = 0; j < fields.length; j++) {
              if (fields[j].charAt(0) !== "*") {
                ...
                if (!onError("Null exclusive interpretation found", i, position)) return;
              };
              position += fields[j].length;
            };
          ...
          function process_data(fields) {
            var double_stops, position = 0;

            if (new_path) {
              if (!onError("Record containing add-spine indicator has not been followed by exclusive interpretation for that spine", i, 0)) return;
              new_path = false;
            };

            if (!first_interpretation_yet) {
              if (!onError("Data record appears before first exclusive interpretation record", i, 0)) return;
              current_no_of_spines = fields.length;
            } else if (current_no_of_spines !== fields.length) {
              if (!onError("Number of tokens in data record does not match the number of currently active spines", i, position)) return;
            };
            ...
            double_stops = fields[j].split(" ");
            if (double_stops.length > 1 && double_stops.indexOf(".") >= 0) {
              if (!onError("Multiple-stop contains null token", i, position)) return;
            };
            position += fields[j].length;
          };
      };
      ...
      for(i = 0; i < hum.length; i++) {
        ...
        if (tokens.length === 1 && tokens[0] === "") {
          if (!onError("Illegal empty record", i, 0)) return;
          continue;
        };
        ...
        if (tokens[0].charAt(0) === "!") {
          process_local(tokens);
        } else if (tokens[0].charAt(0) === "*") {
          process_interpretation(tokens);
        } else {
          process_data(tokens);
        };
      ...
      if (current_no_of_spines !== 0) {
        ...
        if (!onError("All spines have not been properly terminated.", i - 1, 0)) return;
      };
