...
function validateHumdrum_Process(hum, onError, onWarning) {
  ...
  const REGEXP_DURATION = /(\d+)(%\d*)?/;
      ...
      function process_local(fields) {
        var position = 0;
        if (new_path) {
          if (!onError("Record containing add-spine indicator has not followed by exclusive interpretation for that spine",
            i, 0)) return false;
          new_path = false;
        };
        if (!first_interpretation_yet) {
          if (!onError("Local comment precedes first exclusive interpretation record",
            i, 0)) return false;
          current_no_of_spines = fields.length;
        };
        if (fields.length !== current_no_of_spines) {
          if (!onError("Number of sub-comments in local comment does not match the number of currently active spines",
            i, 0)) return false;
        };
        ...
        for(var j = 0; j < fields.length; j++) {
          if (fields[j].charAt(0) === "!") {
            if (fields[j].charAt(1) === "!") {
              if (!onWarning("Local comment may be mistaken for global comment",
                i, position)) return false;
            };
          } else {
            if (!onError("Missing initial exclamation mark in local comment",
              i, position)) return false;
          };
        ...
        for(var j = 0; j < interp.length; j++) {
          if (interp[j].indicator === "N") {
            if (!REGEXP_EXCLUSIVE.test(fields[j])) {
              if (!onError("Record containing add-spine indicator has not been followed by exclusive interpretation for that spine",
                i, 0)) return false;
            };
            interp[j].indicator = "*";
          };
      ...
      function check_first_interp(fields) {
        ...
        for (var j = 0 ; j < fields.length; j++) {
          if (fields[j] === "*") {
            if (!onError("First exclusive interpretation record contains a null interpretation",
              i, position)) return false;
          };
          if (!REGEXP_EXCLUSIVE.test(fields[j])) {
            if (!onError("First exclusive interpretation record contains a non-exclusive interpretation",
              i, position)) return false;
          };
      ...
      function store_new_interps(fields) {
        ...
        for(var j = 0; j < fields.length; j++) {
          ...
          if (REGEXP_EXCLUSIVE.test(fields[j])) {
            if (new_interp.name) {
              if (!onError("Changing spine exclusive interpretation is not allowed",
                i, position)) return false;
            };
      ...
      function store_indicators(fields) {
        ...
        for(var j = 0; j < fields.length && j < interp.length; j++) {
          if (fields[j].charAt(0) !== "*"
              ...
              if (!onError("Spine-path indicators mixed with keyword interpretations",
                i, position)) return false;
          };
          if (fields[j] === "*") {
              interp[j].indicator = "*";
      ...
      function process_indicators() {
        ...
        for(j = 0; j < interp.length; j++) {
          ...
          switch(current.indicator) {
            ...
            case "-": //do_terminate
              if (current.top > 0) {
                if (!onError("Spine terminated " + current.top + "/" + current.bottom + " too early",
                  i, 0)) return false;
              };
              interp.splice(j, 1);
              current_no_of_spines--;
            ...
            case "x": //do_exchange
              ...
              if (error || exchange < 0) {
                if (!onError("Improper number of exchange-path indicators",
                  i, 0)) return false;
              } else {
                temp = current.name;
            ...
            case "v": //do_join
              ...
              if (j === interp.length - 1) {
                if (!onError("Single join-path indicator found at end of interpretation record",
                  i, 0)) return false;
              } else if (interp[j + 1].indicator !== 'v'){
                if (!onError("Join-path indicator is not adjacent to another join-path indicator",
                  i, 0)) return false;
              } else {
                j = j + 1;
                while (j < interp.length && interp[j].indicator === "v") {
                  if (current.name !== interp[j].name) {
                    if (!onError("Exclusive interpretations do not match for designated join spines",
                      i, 0)) return false;
                  };
                  if (current.top * interp[j].bottom !== current.bottom * interp[j].top) {
                    if (!onError("Note durations do not match for designated join spines",
                      i, 0)) return false;
                  };
                  interp.splice(j, 1);
                  current_no_of_spines--;
      ...
      function process_interpretation(fields) {
        ...
        if (new_path) {
          if (!check_new_path(fields)) return false;
        };
        ...
        for (var j = 0; j < fields.length; j++) {
          if (fields[j].charAt(0) !== "*") {
            if (!onError("Missing initial asterisk in interpretation keyword",
              i, position)) return false;
          } else if (fields[j] === "**") {
            if (!onError("Null exclusive interpretation found",
              i, position)) return false;
          };
          position += fields[j].length + 1;
        };
        ...
        if (!first_interpretation_yet) {
          ...
          current_no_of_spines = fields.length;
          if (!check_first_interp(fields) || !store_new_interps(fields)) return false;

          /* Otherwise, determine if the record is a spine path record or not */
        } else {

          if (fields.length !== current_no_of_spines) {
            if (!onError("Incorrect number of spines in interpretation record",
              i, 0)) return false;
            current_no_of_spines = fields.length;
          };
          ...
          if (spine_path_record) {
            if (!store_indicators(fields) || !process_indicators()) return false;
          ...
          /* Otherwise, store the new interpretations */
              ...
              if (!store_new_interps(fields)) return false;
          };
        };
        return true;
      };
      ...
      function process_data(fields) {
        ...
        if (!first_interpretation_yet) {
          if (!onError("Data record appears before first exclusive interpretation record",
            i, 0)) return false;
          current_no_of_spines = fields.length;
        } else if (current_no_of_spines !== fields.length) {
          if (!onError("Number of tokens in data record does not match the number of currently active spines",
            i, position)) return false;
        };
        ...
        for(var j = 0; j < fields.length; j++) {
          ...
          if (j > 0) {
            if (fields[j].charAt(0) === "!") {
              if (fields[j].charAt(1) === "!") {
                if (!onWarning("Data token may be mistaken for global comment",
                  i, position)) return false;
              } else {
                if (!onWarning("Data token may be mistaken for local comment",
                  i, position)) return false;
              };
            } else if (REGEXP_EXCLUSIVE.test(fields[j])) {
              if (!onWarning("Data token may be mistaken for exclusive interpretation",
                i, position)) return false;
            } else if (fields[j].charAt(0) === "*") {
              if (!onWarning("Data token may be mistaken for tandem interpretation",
                i, position)) return false;
            };
          };
           ...
           if (note.charAt(0) !== "=") {
             ...
             if (m = REGEXP_DURATION.exec(note)) {
               ...
               if (!isGrace && spine.top > 0) {
                 if (!onError("Rhythm off by " + spine.top + "/" + spine.bottom + " at field " + (j + 1), i, j)) return false;
               ...
               if (m[1].charAt(0) === "0") {
                 if (bottom > 0) {
                   if (!onError("Non-zero duration starts with zero", i, j)) return false;
                 } else if (m[2]) {
                   if (!onError("Invalid duration denominator", i, j)) return false;
                 };
                 bottom = 1;
                 top = (1 << m[1].length);
               } else {
                 if (m[2] && m[2].length > 1) {
                   ...
                   if (m[2] && !onError("Incorrect percent sign in duration", i, j)) return false;
                   top = 1;
                 };
               };
             };
             ...
             if (!isGrace && m) {
              ...
              if (note.indexOf("%") >= 0) {
                if (!onError("Invalid percent sign", i, j)) return false;
              };
        ...
        if (subtokens.length > 1 && subtokens.indexOf(".") >= 0) {
          if (!onError("Multiple-stop contains null token", i, position)) return false;
        };
        position += fields[j].length + 1;
      };
      ...
      if (min_top > 0) {
        ...
        for(j = 0; j < fields.length && j < interp.length; j++) {
          ...
          if (spine.top < 0) {
            ...
            if (!onError("Missing note at field " + (j + 1) + " " + fields[j], i, position)) return false;
            spine.top = 0; spine.bottom = 1;
          };
        };
      };
  ...
  function process_tokens(fields) {
    ...
    for(var j = 0; j < fields.length; j++) {
      if (REGEXP_EMPTY.test(fields[j])) {
        if (!onError("Illegal empty token", i, position)) return;
      };
      position += fields[j].length + 1;
    };
    return true
  ...
  for(i = 0; i < hum.length; i++) {
    ...
    if (tokens.length === 1 && tokens[0] === "") {
      if (!onWarning("Illegal empty record", i, 0)) return;
      continue;
    };
    ...
    if (tokens[0].charAt(0) === "!") {
      if (!process_local(tokens)) return;
    } else if (tokens[0].charAt(0) === "*") {
      if (!process_interpretation(tokens)) return;
    } else {
      if (!process_data(tokens)) return;
    };
