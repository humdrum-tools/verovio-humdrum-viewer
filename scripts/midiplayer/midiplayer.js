...
function convertFile(file, data, starttime) {
...
function play(starttime) {
	if (starttime) {
		var samples = starttime * SAMPLE_RATE / 1000.0;
		midiPlayer_currentSamples = samples;
		// midiPlayer_currentSamples = Math.min(midiPlayer_totalSamples, samples);
console.log("SETTING CURRENT SAMPLES TO ", midiPlayer_currentSamples, "SAMPLERATE", SAMPLE_RATE);
	}
...
(function ($) {
	...
	$.fn.midiPlayer = function (options) {
		...
		$.fn.midiPlayer.play = function (song, starttime) {
			...
			if (midiPlayer_isLoaded == false) {
				...
				if (midiPlayer_totalSamples > 0) {
					stop();
					// A timeout is necessary because otherwise writing to the
					// disk is not done:
					setTimeout(function () {
							convertFile("player.midi", byteArray, starttime);
						}, 200);
				} else {
					setTimeout(function () {
							convertFile("player.midi", byteArray, starttime);
